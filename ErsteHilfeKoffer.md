...
### Probleme mit den C-Pretty-Printer oder clang-tity
- Wir verwenden den Language Server clangd. Dieser bindet Bibliotheken, die die clang-format und clang-tidy Funktionalität zur Verfügung stellen, an. Wenn die Ansteuerung von clang-tidy und clang-format über die Python Skripte funktioniert, aber zum Beispiel keine Warnungen im Problem Fenster angezeigt werden oder der Pretty-Printer für die aktuelle Datei im Text-Editor nicht aufgerufen werden kann, dann ist eventuell clangd nicht installiert. Überprüfe via clangd --version, ob clangd installiert ist bzw. gefunden wird. 
